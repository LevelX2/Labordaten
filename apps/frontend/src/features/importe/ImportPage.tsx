...
import {
  ...
  formatImportReferenceResolved,
  formatMappingInfo,
  getAliasRecommendation,
  getGruppenVorschlagDefault,
  getIgnoredMeasurementCount,
  getImportChecksBySeverity,
  ...
  type MappingFilterMode,
  type ParameterDialogFilterMode
} from "./importMapping";
...
export function ImportPage() {
  ...
  const createDraftMutation = useMutation({
    ...
    onSuccess: async (detail) => {
      ...
      await queryClient.invalidateQueries({ queryKey: queryKeys.importe });
      await queryClient.invalidateQueries({ queryKey: queryKeys.importDetail(detail.id) });
    }
  });
  ...
  const uebernehmenMutation = useMutation({
    ...
    onSuccess: async (detail) => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.importe }),
        queryClient.invalidateQueries({ queryKey: queryKeys.importDetail(detail.id) }),
  ...
  const komplettEntfernenMutation = useMutation({
    ...
    onSuccess: async (response) => {
      queryClient.setQueryData<ImportVorgangListItem[]>(queryKeys.importe, (current) =>
        current?.filter((item) => item.id !== response.import_id) ?? current
      );
      queryClient.removeQueries({ queryKey: queryKeys.importDetail(response.import_id) });
      setShowDiscardPanel(false);
      setRemoveLinkedDocument(false);
  ...
  const importPruefentscheidungMutation = useMutation({
    ...
    onSuccess: async (detail) => {
      queryClient.setQueryData(queryKeys.importDetail(detail.id), detail);
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.importe }),
        queryClient.invalidateQueries({ queryKey: queryKeys.importDetail(detail.id) })
      ]);
  ...
  const gruppenVorschlaegeMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.gruppen }),
        queryClient.invalidateQueries({ queryKey: queryKeys.importDetail(selectedImportId) })
      ]);
    }
  });
