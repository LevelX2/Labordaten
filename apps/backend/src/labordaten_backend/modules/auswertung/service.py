...
def build_auswertung(db: Session, payload: AuswertungRequest) -> AuswertungResponse:
    ...
    references = _load_reference_map(db, [messwert.id for messwert, _, _, _ in rows])
    overrides_by_parameter = _load_target_override_map(db, payload.person_id)
    targets_by_parameter = _load_target_map(db, {parameter.id for _, _, parameter, _ in rows})

    grouped: dict[str, list[tuple[Messwert, Befund, Laborparameter, Labor | None]]] = defaultdict(list)
    for row in rows:
    ...
    for parameter_id, parameter_rows in sorted(grouped.items(), key=lambda item: item[1][0][2].anzeigename.lower()):
        ...
        for messwert, befund, _, labor in sorted(parameter_rows, key=lambda item: _effective_date(item[1], item[0]) or date.min):
            ...
            zielbereich = _select_applicable_target(
                ...
                override=overrides_by_parameter.get(parameter_id),
    ...
    return AuswertungResponse(person_id=payload.person_id, serien=serien)
...
def _execute_measurement_query(db: Session, payload: AuswertungRequest):
    stmt: Select = (
        ...
        .where(Messwert.person_id == payload.person_id)
    )
    ...
    stmt = stmt.order_by(Laborparameter.anzeigename.asc(), Befund.entnahmedatum.asc(), Messwert.erstellt_am.asc())
...
def _load_target_override_map(db: Session, person_id: str) -> dict[str, dict[str, float | str | None]]:
    stmt = (
