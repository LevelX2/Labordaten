...
- Bei Einheiten soll die KI die Kerneinheit erfassen und reine Fußnotenmarker, Tabellenmarker oder Verweiszeichen am Einheitenende nicht als Einheit übernehmen. Beispiel: `µmol/l*` wird als `µmol/l` in `einheitOriginal` und `referenzEinheit` ausgegeben; die Fußnote selbst gehört nur bei inhaltlicher Relevanz in Bemerkung oder KI-Hinweis.
...
- Der Prompt stellt zusätzlich klar, dass bei Einheiten die Kerneinheit zählt und Fußnotenmarker wie `*` am Ende nicht in `einheitOriginal` oder `referenzEinheit` übernommen werden sollen.
