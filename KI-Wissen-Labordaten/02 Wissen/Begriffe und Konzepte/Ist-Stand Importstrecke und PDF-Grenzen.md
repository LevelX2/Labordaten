...
- Bei Einheiten soll der Prompt die Kerneinheit bevorzugen: Fußnotenmarker oder Tabellenverweise am Ende einer Einheit, zum Beispiel `µmol/l*`, sollen nicht in `einheitOriginal` oder `referenzEinheit` übernommen werden.
...
- Für neue Parameter-Vorschläge soll `beschreibungKurz` als erwartete Kurzbeschreibung der allgemeinen fachlichen Bedeutung geliefert werden. Sie bleibt strikt berichtsunabhängig: Berichtskommentare, Zusatzuntersuchungs-Empfehlungen, Einsendehinweise oder patientenbezogene Risikohinweise bleiben Messwertbemerkung beziehungsweise KI-/Dokumentbegründung und werden nicht als Parameterbeschreibung gespeichert. Der Prompt fordert die externe KI inzwischen ausdrücklich auf, dafür kurz zu recherchieren beziehungsweise allgemeines Laborwissen zu nutzen; leer bleiben soll die Beschreibung nur, wenn auch daraus keine belastbare Bedeutung ableitbar ist. Bei bestätigter Neuanlage wird diese Kurzbeschreibung als Parameterbeschreibung übernommen.
...
- Bereits vorhandene Parameter-Aliasse werden im KI-Prompt als Teil des Parameterkontexts mitgegeben. Wenn ein Originalname im Bericht bereits als bekannter Alias vorkommt, soll die KI die vorhandene `parameterId` verwenden und keine erneute Alias-Übernahme vorschlagen. Die Alias-Identität ist inzwischen einheitenbewusst: Derselbe Aliasname darf für verschiedene Parameter vorkommen, wenn die führenden Einheiten verschieden sind. Beim automatischen Alias-Matching wird die Import-Einheit zur Disambiguierung genutzt; bei gleicher oder unklarer Einheit wird eine doppelte Alias-Anlage weiter verhindert. Harte Alias-Fehler bleiben für Kollisionen mit Anzeigenamen, internen Schlüsseln oder fehlendem Zielparameter bestehen.
...
- Das Alias-Mapping berücksichtigt inzwischen die Import-Einheit, damit gleich benannte Laborwerte mit unterschiedlichen führenden Einheiten nicht automatisch auf denselben kanonischen Parameter laufen. Diese Logik ersetzt keine freie Matrix- oder Materialmodellierung; bei gleicher Einheit, unklarer Einheit oder fehlendem Materialkontext bleibt weiterhin eine bewusste Prüfung nötig.
