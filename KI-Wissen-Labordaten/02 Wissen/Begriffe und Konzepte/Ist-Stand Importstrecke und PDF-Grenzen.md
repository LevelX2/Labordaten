...
- Bereits vorhandene Parameter-Aliasse werden im KI-Prompt als Teil des Parameterkontexts mitgegeben. Wenn ein Originalname im Bericht bereits als bekannter Alias vorkommt, soll die KI die vorhandene `parameterId` verwenden und keine erneute Alias-Übernahme vorschlagen. Wenn das KI-JSON dennoch `aliasUebernehmen` setzt und der Alias bereits bei einem anderen Parameter existiert, wird der Alias nicht erneut angelegt und nur noch als Warnung angezeigt; die Messwertzuordnung kann mit bestätigten Warnungen übernommen werden. Harte Alias-Fehler bleiben für Kollisionen mit Anzeigenamen, internen Schlüsseln oder fehlendem Zielparameter bestehen.
